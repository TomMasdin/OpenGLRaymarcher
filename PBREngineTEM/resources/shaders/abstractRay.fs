...
vec2 oprep2(vec2 p, float q, float s, float k) {
	float r = 1.0 / q;
	float ofs = s;
...
float map(vec3 p) {
	...
	p.xy = oprep2(p.xy, 6.0, 0.45, iTime);
...
// Spiral rotation for color highlight
...
vec3 getShading(vec3 p, vec3 normal, vec3 lightPos) {
	...
	col *= 4.0 * saw(p.z, 0.092, 0.77, fid * 2.5); // 2.5
